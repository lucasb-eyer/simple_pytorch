...
def main(rank, local_rank, world_size):
    ...
    model = MyMLP(depth=1).to("cuda")
    printmem("model.to(cuda)", rank)

    # Since I haven't set a seed, here they DIFFER ACROSS RANKS.
    ...
    # According to `distribute_tensor` doc-comment, I expected them to now all be the same
